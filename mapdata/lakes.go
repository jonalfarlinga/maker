...
func (m *MapArray) TerraformLakes(fillin int) {
	scope := max(min(m.Width, m.Height)/50, 5)
	x1, y1 := 0, 0
	x2, y2 := scope, scope
	...
	scanRow := func(x1, y1, x2, y2 int) {
		// Calculate the number of perimeter land cells
		perimeterLand := 0
		for i := x1; i < x2; i++ {
		...
		// Calculate the number of contained water cells
		containedWater := 0
		for i := 1; i < scope; i++ {
			for j := 1; j < scope; j++ {
				if m.mapArray[i][j] == 0 {
		...
		for x2 < m.Width {
			...
			if perimeterLand == perimeterSize {
				// It's a lake
...
func makeALake(m *MapArray, x1, y1, x2, y2 int) {
